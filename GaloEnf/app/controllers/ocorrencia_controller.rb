class OcorrenciaController < ApplicationController
  ...
  def create
    @ocorrencium = Ocorrencia.new(ocorrencium_params)

    respond_to do |format|
      if @ocorrencium.save
        format.html { redirect_to @ocorrencium, notice: 'Ocorrencia was successfully created.' }
  ...
  def update
    respond_to do |format|
      if @ocorrencium.update(ocorrencium_params)
        format.html { redirect_to @ocorrencium, notice: 'Ocorrencia was successfully updated.' }
  ...
  def destroy
    @ocorrencium.destroy
    respond_to do |format|
      format.html { redirect_to ocorrencia_index_url, notice: 'Ocorrencia was successfully destroyed.' }
  ...
  private
    ...
    def set_ocorrencium
      @ocorrencium = Ocorrencia.find(params[:id])
    end

    # Never trust parameters from the scary internet, only allow the white list through.
    def ocorrencium_params
      params.require(:ocorrencium).permit(:data, :hora, :ocorrido, :procedimento)
    end
end
